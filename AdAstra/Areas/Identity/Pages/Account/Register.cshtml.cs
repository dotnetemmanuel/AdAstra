...
namespace AdAstra.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private AdAstraUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<AdAstraUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(AdAstraUser)}'. " +
                    $"Ensure that '{nameof(AdAstraUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<AdAstraUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<AdAstraUser>)_userStore;
        }
    }
}
